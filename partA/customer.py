...
class Customer:
    ...
    (self.Ctype, self.lifestyle, self.vacation, self.eCredit, self.salary, self.prop, self.cla, self.predC, self.sim)
...
class Knn:
    ...
    def randomSelect():
        ...
        #for i in range(int(36*num)):
            #item1 = train_total[np.random.randint(0,36)]
            #item1 = copy_train
            #item5 = train_total[np.random.randint(-36,-1)]
            #if item1 not in train_C1:
             #   train_C1.append(item1)
            #else:
                #i -= 1
               # copy_train.remove(item1)
            #if item5 not in train_C5:
              #  train_C5.append(item5)
            #else:
               # i -= 1
       # for i in range(int(26*num)):
           # item2 = train_total[np.random.randint(36,61)]
           # if not item2 in train_C2:
           #     train_C2.append(item2)
           # else:
           #     i -= 1
       # for i in range(int(41*num)):
          #  item3 = train_total[np.random.randint(62,102)]
          #  if not item3 in train_C3:
           #     train_C3.append(item3)
           # else:
           #     i -= 1
        #for i in range(int(47*num)):
         #   item4 = train_total[np.random.randint(103,149)]
         #   if not item4 in train_C4:
          #      train_C4.append(item4)
          #  else:
           #     i -= 1
...
#     print(t)
    ...
    #print("t.prop",t.prop)
    if i == 19:
        print("norm 19",Knn.listTrainCustomer[i])
#     print('after:')
#     print(t)
...
w1, w2, w3, w4, w5, w6 = 1,0.1,2,5,4,5
near = []
...
for row in Knn.listTrainCustomer:
    print("TRAIN",row.cla)           
for row in Knn.listTestCustomer:
    print("TESTOri", row.cla)
    print("TEST",row.predC)
count = 0;
for row in Knn.listTestCustomer:
